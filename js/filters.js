...
function readFilterState() {
  ...
  // Global customer filter is a searchable multiselect dropdown (see widgets.js).
  // SELECTED_CUSTOMER_KEYS holds normalized customer keys; empty set = all customers.
  const customerKeys = new Set(typeof SELECTED_CUSTOMER_KEYS !== "undefined" ? SELECTED_CUSTOMER_KEYS : []);
  ...
  return {
    ...
    customerKeys, saeSet, deliveryStatus: "", ageingMin, minValue, paymentVia
  };
}
...
function filterBills(bills, f) {
  return bills.filter(b =>
    ...
    (!f.customerKeys || f.customerKeys.size === 0 || f.customerKeys.has(b.customerKey)) &&
...
function filterPayments(payments, f) {
  return payments.filter(p =>
    ...
    (!f.customerKeys || f.customerKeys.size === 0 || f.customerKeys.has(p.customerKey))
  );
}
...
function filterReturns(returns, f) {
  return returns.filter(s =>
    ...
    (!f.customerKeys || f.customerKeys.size === 0 || f.customerKeys.has(s.customerKey))
  );
}
...
function filterOrderLines(lines, f) {
  return lines.filter(l =>
    ...
    (!f.customerKeys || f.customerKeys.size === 0 || f.customerKeys.has(l.customerKey))
  );
}
