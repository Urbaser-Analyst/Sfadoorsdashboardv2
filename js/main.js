...
function populateFilterOptions(DB) {
  ...
  const statusSelect = document.getElementById("f_deliveryStatus");
  statusSelect.innerHTML = `<option value="">All</option>` + DB.deliveryStatusList.map(m => `<option value="${m}">${m}</option>`).join("");
...
function wireControls() {
  ...
  const filterIds = ["f_dateFrom", "f_dateTo", "f_customerSearch", "f_sae", "f_deliveryStatus", "f_minValue", "f_paymentVia"];
  ...
  document.getElementById("clearFiltersBtn").addEventListener("click", () => {
    ...
    document.getElementById("f_deliveryStatus").value = "";
