...
const productAnalysisState = {
  ...
  dims: new Set(), designs: new Set(), majorColors: new Set(), minorColors: new Set(),
...
function renderProducts(DB, f) {
  ...
  if (productAnalysisState.dims.size) lines = lines.filter(l => productAnalysisState.dims.has(l.dimensions));
  if (productAnalysisState.designs.size) lines = lines.filter(l => productAnalysisState.designs.has(l.designs));
  if (productAnalysisState.majorColors.size) lines = lines.filter(l => productAnalysisState.majorColors.has(l.majorColor));
  if (productAnalysisState.minorColors.size) lines = lines.filter(l => productAnalysisState.minorColors.has(l.minorColor));
  ...
  document.getElementById("viewRoot").innerHTML = `
    <div class="panel">
      ...
      <div class="inline-filterbar">
        <div class="filter-group"><label>Dimensions</label>${multiSelectHtml("pf_dims", DB.dimensionList, productAnalysisState.dims)}</div>
        <div class="filter-group"><label>Designs</label>${multiSelectHtml("pf_designs", DB.designList, productAnalysisState.designs)}</div>
        <div class="filter-group"><label>Major Color</label>${multiSelectHtml("pf_major", DB.majorColorList, productAnalysisState.majorColors)}</div>
        <div class="filter-group"><label>Minor Color</label>${multiSelectHtml("pf_minor", DB.minorColorList, productAnalysisState.minorColors)}</div>
  ...
  bindMulti("pf_dims", productAnalysisState.dims);
  bindMulti("pf_designs", productAnalysisState.designs);
  bindMulti("pf_major", productAnalysisState.majorColors);
  bindMulti("pf_minor", productAnalysisState.minorColors);
  ...
  document.getElementById("pf_clear").addEventListener("click", () => {
    ...
    productAnalysisState.dims.clear(); productAnalysisState.designs.clear();
    productAnalysisState.majorColors.clear(); productAnalysisState.minorColors.clear();
...
/* Each Entry Statement — chronological, running-balance view of every order,
   payment, and stock return, per customer. Previously shown inside the
   Customer Master drill-down modal; now lives here on All Activity only. */
function buildEachEntryStatement(orders, payments, returns) {
  const byCustomer = {};
  const ensure = key => (byCustomer[key] = byCustomer[key] || []);
  orders.forEach(b => ensure(b.customerKey).push({
    type: b.isDelivered ? "Delivered" : "Not Delivered", ref: "Order", date: b.billDate, id: b.orderId,
  ...
  payments.forEach(p => ensure(p.customerKey).push({
    type: "Payment", ref: "Payment", date: p.date, id: p.id, quotationNo: "—", value: p.amount,
    orderValue: 0, receiptValue: p.amount, customerName: p.customerName, customerKey: p.customerKey, mobile: p.mobile || "", sae: p.sae
  }));
  returns.forEach(r => ensure(r.customerKey).push({
    type: "Return", ref: "Return", date: r.date, id: r.id, quotationNo: "—", value: r.amount,
    orderValue: 0, receiptValue: r.amount, customerName: r.customerName, customerKey: r.customerKey, mobile: r.mobile || "", sae: r.sae
  }));
  ...
  Object.values(byCustomer).forEach(events => {
    ...
    events.forEach(e => {
      ...
      const action = e.ref === "Payment" ? `We have received a payment of ${fmtRupee(e.value)} on ${formatDate(e.date)}.`
        : e.ref === "Return" ? `We have received stock return of ${fmtRupee(e.value)} on ${formatDate(e.date)}.`
        : `Your bill ${e.id} dated ${formatDate(e.date)} has been generated for ${fmtRupee(e.value)}.`;
      allRows.push({
        ...e, mobile,
        cumulativeOrderValue: +cOrders.toFixed(2), cumulativeReceipts: +cReceipts.toFixed(2),
        balance, message: `${action} Updated balance: ${fmtRupee(balance)}.`
      });
    });
  });
...
function renderActivity(DB, f) {
  ...
  document.getElementById("viewRoot").innerHTML = `
    ...
    <div class="panel">
      <div class="panel-header">
        ...
        <div class="panel-note">Every order, payment, and stock return, in date order per customer, with a running Order Value / Payments + Return / Outstanding Balance and a plain-language message. Moved here from the Customer Master drill-down.</div>
      </div>
  ...
  renderDataTable("statementTable", {
    ...
    columns: [
      ...
      { key: "message", label: "Message" }, { key: "sae", label: "SAE" }
    ],
    rows: statementRows,
    onRowClick: row => openCustomerLedger(DB, row.customerKey)
  });
